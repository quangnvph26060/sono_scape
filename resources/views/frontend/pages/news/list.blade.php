                    ...
                    <div class="blog-item post-list-view" data-wow-delay="0.25s">
                                    ...
                                    <div class="relative img-post">
                                        <a href="{{ route('news.detail', $news->first()->slug) }}"
                                            ...
                                            <img src="{{ showImage($news->first()->featured_image) }}" width="100%"
                                                height="100%" data-isrc="{{ showImage($news->first()->featured_image) }}"
                        ...
                        <div class="row">
                            @foreach ($news->skip(1) as $item)
                                <div class="item col-12 col-sm-6 col-md-6 col-lg-6"> <!-- Cập nhật đây để chỉ chiếm 50% -->
                                    <div class="post-item relative" data-id="224">
                                        <figure class="photoframe relative">
                                            <div class="relative img-post">
                                                <a href="{{ route('news.detail', $item->slug) }}" class="d-block relative text-center">
                                                    <img src="{{ showImage($item->featured_image) }}" width="100%" height="100%"
                                                        data-isrc="{{ showImage($item->featured_image) }}" class="lazyload"
                                                        alt="{{ $item->subject }}" aria-label="{{ $item->subject }}" />
                                                </a>
                                            </div>
                                        </figure>
                                        ...
                                        <figcaption class="info-post" style="padding: 0 10px 10px 10px">
                                            ...
                                            <hr>
                                            <div class="wrap-two-lines post-title">
                                                <a href="{{ route('news.detail', $item->slug) }}" class="two-lines" aria-label="{{ $item->subject }}">
                                                    {{ $item->subject }}.
                                                </a>
                                            </div>
